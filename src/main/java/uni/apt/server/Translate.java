...
public class Translate {
    ...
    public static class TranslatedDocument{
    ...
    public List<TranslatedDocument> translate(@RequestBody String body){
        ...
        BsonArray arr = data.getArray("items");
    ...
    private static TranslatedDocument handle(BsonDocument doc , List<QuerySelector> selector){
        ...
        temp.paragraph = OnlineDB.getParagraph(doc.getInt64("paragraphID").getValue());
        for (QuerySelector q : selector){
            handle(q , temp);
        }

        TranslatedDocument ret = new TranslatedDocument();
        ret.title = OnlineDB.getParagraph(doc.getInt64("titleID").getValue());
        ...
        if (temp.boldRanges.size() > 0) {
            ...
            for (Range r : newRanges){
                int s = r.start - sub_start + 3;
                int e = r.end - sub_start + 3;
                ...
                ret.paragraphs.add(temp.paragraph.substring(prev.end - sub_start + 3 , s));
                ret.bold.add(false);
